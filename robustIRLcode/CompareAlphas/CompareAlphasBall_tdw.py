...
if compute:
    ...
    env_2p = TwoDangerEnvironment(env_type,dim, prop = float(noiseL))
    ...
    save_result(data, "BallCompareAlphas_tdw_env" + str(env_type) + "size" + str(dim)+"lr"+str(lr)+noiseL+"alphaE"+alphaE+"fix_start"+str(fix_start))

else:
    data = load_result( "BallCompareAlphas_tdw_env" + str(env_type) + "size" + str(dim)+"lr"+str(lr)+noiseL+"alphaE"+alphaE+"fix_start"+str(fix_start))
...
for i,enoise in enumerate(noisesE):
    noisesE[i] = float(enoise) - args.noiseL
...
plot_lines( list_to_plot = Vs_to_plot,
            ...
            folder = "",
            title = "BallCompare Alphas_tdw Env "+str(env_type)+" noise L "+ noiseL+"dim"+str(dim)+"alphaE"+alphaE+"fix_start"+str(fix_start),
            x_axis = noisesE)
