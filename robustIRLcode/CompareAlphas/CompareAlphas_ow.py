...
def base_dir(alphaE, alphaL, noiseE, noiseL, seed, dim, linear, reg_opp, fix_start, fix_horizon):
    softQ_lr = 0.0
    folder = "../../models_ow"
    dir_path = folder + '/fix_horizon'+ str(fix_horizon) + '/fix_start'+ str(fix_start) +"/dim_" + str(dim)+ "/Linear"+str(linear)+"/RegOpp"+str(reg_opp)+"/alphaE_" + str(alphaE) + "_alphaL_" + str(alphaL) + \
            "/beta_0.0_beta_op_0.0/softQ_lr_" + str(softQ_lr) + "/noiseE_" + str(noiseE) + "_noiseL_" + str(noiseL) + \
            "/seed_" + str(seed) + "/0"   
    return dir_path

def load_multiple_data(lr, alphasE, alphasL, noisesE, noisesL, seed_env, dim, linear, reg_opp, fix_start, fix_horizon):
    results = {}
...
### CompareAlphas
if compute:
    results = load_multiple_data(lr, alphasE, alphasL, noisesE, noisesL, [args.env_seed], dim, linear, reg_opp, fix_start, fix_horizon)   
    policies = []
    for alphaL in alphasL:
        for noiseE in noisesE:
    ...
    save_result(data, "YT_OWCompareAlphas_env" + str(args.env_seed) + "size" + str(dim)+"lr"+str(lr)+noiseL +"alphaE"+alphaE+"fix_start"+str(fix_start))

else:
    data = load_result( "YT_OWCompareAlphas_env" + str(args.env_seed) + "size" + str(dim)+"lr"+str(lr)+noiseL +"alphaE"+alphaE+"fix_start"+str(fix_start))
...
plot_lines( list_to_plot = Vs_to_plot,
            ...
            folder = "",
            title = "OWNo0.6ConvergedCompare Alphas Env "+ str(args.env_seed) +" noise L "+ noiseL+"dim"+str(dim)+"alphaE"+alphaE+"fix_start"+str(fix_start),
            x_axis = noisesE)
