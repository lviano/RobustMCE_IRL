...
mode = args.mode
dim = args.dim
twodangerworld = TwoDangerEnvironment(mode, dim, prop=args.noiseE)
solver = MDPsolver(twodangerworld)
...
n_traj = 10000
teacher = Agent(twodangerworld, policy = player)
if fixed_start:
    start = dim*dim - 1
else:
    start = None
...
rand_pi = player
p_initial = compute_initial_probabilities(demon_states, twodangerworld)

if fixed_start:
    mu_teacher = solver.mu_policy(solver.policy,stochastic=True, emp_p_in=p_initial, no_one_hot=True)
else:
    mu_teacher = solver.mu_policy(solver.policy,stochastic=True, no_one_hot=True)

learned_twodangerworld = TwoDangerEnvironment(mode, dim, prop=args.noiseL)
learned_Solver = MDPsolver(learned_twodangerworld)
...
if not args.linear:
    ...
    policies, player, adv,  reward, err, v = IRL.deep_max_ent(base_dir, optimizer, reward_nn, p_initial, args.beta, args.beta_op, args.alphaL, args.softQ_lr, IRL_lr = args.lr, reg_opp = args.reg_opp, no_one_hot=True)
else:
    policies, player, adv,  reward, err, v = IRL.max_ent_irl(base_dir, lr=args.lr,emp_p_in = p_initial, beta = args.beta, beta_op = args.beta_op, softQ_lr = args.softQ_lr, two_players=True, alpha = args.alphaL,  reg_opp = args.reg_opp, no_one_hot=True)
